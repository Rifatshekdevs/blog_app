...
import 'package:news_api/src/configs/api_exception.dart';
import 'package:news_api/src/configs/text_enum.dart';
...
class ApiClient {
  ...
  static _handleTimeoutException(
    ...
    if (onError != null) {
      onError(ApiException(
        message: TextEnum.serverNotResponding.tr,
        url: url,
      ));
    } else {
      _handleError(TextEnum.serverNotResponding.tr);
    }
  }
  ...
  static _handleSocketException(
    ...
    if (onError != null) {
      onError(ApiException(
        message: TextEnum.noInternetConnection.tr,
        url: url,
      ));
    } else {
      _handleError(TextEnum.noInternetConnection.tr);
    }
  }
  ...
  static _handleDioError(
    ...
    if (error.response?.statusCode == 404) {
      if (onError != null) {
        return onError(ApiException(
          message: TextEnum.urlNotFound.tr,
          url: url,
          statusCode: 404,
        ));
      } else {
        return _handleError(TextEnum.urlNotFound.tr);
      }
    }
    ...
    if (error.message!.toLowerCase().contains('socket')) {
      if (onError != null) {
        ...
        return _handleError(TextEnum.noInternetConnection.tr);
      }
    }
    ...
    if (error.response?.statusCode == 500) {
      var exception = ApiException(
        message: TextEnum.serverError.tr,
        url: url,
        statusCode: 500,
      );
